    ...
    while True:  # 메인 무한 루프
        try:
            ...
            while True:
                ...
                # 시장 개장 시간에 5분마다 손절매 체크 - 추가
                if is_market_time() and stop_loss_minutes_elapsed >= 5:
                    send_message("손절매 조건 확인 중...")
                    for symbol in SYMBOLS:
                        try:
                            stop_loss_triggered = check_stop_loss(symbol, STOP_LOSS_PERCENT)
                            if stop_loss_triggered:
                                send_message(f"{symbol} 손절매 완료", symbol)
                        except Exception as e:
                            send_message(f"손절매 체크 중 오류: {str(e)}", symbol)
                ...
                if force_first_check or (minutes_elapsed >= 19 and time_to_check):
                    ...
                    for symbol in SYMBOLS:
                        try:
                            ...
                            # 1. 먼저 손절매 조건 확인 (RSI 기반 매매 전에) - 추가
                            stop_loss_triggered = check_stop_loss(symbol, STOP_LOSS_PERCENT)
                            if stop_loss_triggered:
                            ...
                            # 매도 조건
                                ...
                                try:
                                    ...
                                    if symbol in stock_dict:
                                        # 현재 보유 주식 정보 가져오기
                                        stock_info = stock_dict[symbol]
                                        profit_rate = float(stock_info['profit_rate'])  # 손익률
                                        current_price_from_balance = float(stock_info['current_price'])
                                        purchase_price = float(stock_info['purchase_price'])
                                        qty = stock_info['qty']
                                        
                                        send_message(f"보유 정보 - 매입가: ${purchase_price:.2f}, 현재가: ${current_price_from_balance:.2f}, 손익률: {profit_rate:.2f}%", symbol)
                                        
                                        # 수익이 플러스일 때만 매도, 마이너스일 때는 홀드
                                        if profit_rate > 0:
                                            send_message(f"✅ 수익 상태이므로 매도를 진행합니다 (수익률: +{profit_rate:.2f}%)", symbol)
                                            try:
                                                sell_result = sell(code=symbol, qty=str(qty), price=str(current_price))
                                                if sell_result:
                                                    send_message(f"✅ {symbol} 익절 매도 완료", symbol)
                                            except Exception as e:
                                                if 'access_token' in str(e).lower():
                                            ...
                                            send_message(f"⏸️ 손실 상태이므로 매도하지 않고 홀드합니다 (손실률: {profit_rate:.2f}%)", symbol)
                                            send_message(f"💡 현재 손실이지만 RSI가 과매수 구간이므로 추가 상승 가능성을 기다립니다", symbol)
                                    else:
