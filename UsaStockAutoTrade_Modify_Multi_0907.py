...
MARKET = "NASD"  # 나스닥
EXCD_MARKET = "NAS"
SYMBOLS = ["PLTR", "NVDA","IONQ","TSLA","CRCL"]  # 여러 심볼 추가 "IONQ" 제외외 "TSLA"
...
# 손절매 설정
STOP_LOSS_PERCENT = 10             # 손절매 기준: 하락률 (%) - 음수로 입력하지 마세요

# 매도 설정
PROFIT_TAKE_PERCENT = 10.0          # 익절 기준: 수익률 (%) - 양수로 입력하세요

# 매수 설정
BUY_RATIO = 0.30                   # 보유 현금 대비 매수 비율 (0.30 = 30%)
...
def wait_for_market_open():
    ...
    try:
        ...
        if not refresh_token():  # 시장 개장 시 토큰 갱신
            send_message("토큰 갱신에 실패했습니다. 1분 후 다시 시도합니다.", level=MESSAGE_LEVEL_CRITICAL)
            time.sleep(60)
            refresh_token()
    except Exception as e:
...
def should_sell(analysis, profit_rate):
    """
    매도 조건 판단 (RSI 설정값 이상 + 수익률 5% 이상일 때)
    analysis: get_technical_analysis 결과
    profit_rate: 현재 수익률
    """
    if not analysis:
        ...
        try:
            ...
            while True:
                ...
                if is_market_time() and stop_loss_minutes_elapsed >= STOP_LOSS_CHECK_INTERVAL:
                    send_message("손절매 조건 확인 중...", level=MESSAGE_LEVEL_DEBUG)
