...
def generate_daily_summary_messages(symbol):
    ...
    message1 += f"종가(마감): ₩{final_price:,.0f} | RSI: {final_rsi:.2f} | {MA_PERIOD}일 이평: ₩{final_ma:,.0f}\n"
    ...
    if key_points:
        ...
        for point in key_points:
            ...
            message2 += f"{point['time']} | RSI {point['rsi']:.2f} | ₩{point['current_price']:,.0f} | "
            message2 += f"{MA_PERIOD}일 이평 ₩{point['ma_value']:,.0f} → {signal_status}\n"
    ...
    if data['trades']:
        for trade in data['trades']:
            trade_type_emoji = "🟢" if trade['type'] == 'buy' else "🔴"
            message3 += f"{trade_type_emoji} {trade['time']} | {trade['type'].upper()} {trade['qty']}주 @ ₩{trade['price']:,.0f}\n"
    else:
...
def calculate_rsi(data, periods=RSI_PERIOD):
    try:
        ...
        # print(f"✅ RSI 계산 완료: {latest_rsi}")  # 로그 스팸 방지
        return latest_rsi
    except Exception as e:
