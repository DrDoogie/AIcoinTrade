...
# 트레이딩 제약 조건 설정 
TRADING_CONFIG = {
    'MAX_TRADE_PERCENTAGE': 1.0,    # 최대 거래 비중
...
def init_database():
    try:
        conn = sqlite3.connect('bitcoin_trades.db')  #trading_bot.db
        cursor = conn.cursor()
...
def collect_market_data():
    ...
    try:
        # 1분봉과 5분봉 데이터 수집
        df_1m = pyupbit.get_ohlcv("KRW-BTC", interval="minute1", count=2880)  #100
        df_5m = pyupbit.get_ohlcv("KRW-BTC", interval="minute5", count=576) #20
    ...
    try:
        ...
        if order:
            ...
            trade_data = {
                ...
                'take_profit_price': current_price * 1.025,  # 2.5% 익절
