...
TRADING_CONFIG = {
    ...
    'RSI_LOWER_BOUND': 33,          # RSI 하한선
    'RSI_UPPER_BOUND': 70,          # RSI 상한선
...
if not all([access, secret,  DISCORD_WEBHOOK_URL]): #[access, secret, openai_api_key, DISCORD_WEBHOOK_URL]):
...
def collect_market_data():
    ...
    try:
        # 1분봉과 5분봉 데이터 수집
        df_10m = pyupbit.get_ohlcv("KRW-XRP", interval="minute10", count=100)
        df_20m = pyupbit.get_ohlcv("KRW-XRP", interval="minute20", count=20)
        
        if df_10m is None or df_20m is None:
            logger.error("OHLCV 데이터 수집 실패")
            return None
            
        # 기술적 지표 계산
        df_10m = calculate_indicators(df_10m)
        df_20m = calculate_indicators(df_20m)
        ...
        market_data = {
            ...
            'rsi': float(df_10m['rsi'].iloc[-1]),
            'bb_width': float(df_10m['bb_width'].iloc[-1]),
            'macd': float(df_10m['macd'].iloc[-1]),
            'macd_signal': float(df_10m['macd_signal'].iloc[-1]),
            'ema_short': float(df_10m['ema_short'].iloc[-1]),
            'ema_long': float(df_10m['ema_long'].iloc[-1]),
            'volatility': float(df_10m['volatility'].iloc[-1]),
            'trend': calculate_trend(df_20m),
            'df_10m': df_10m,
            'df_20m': df_20m
        }
...
def execute_buy(percentage, market_data):
    ...
    try:
        ...
        if order:
            ...
            # 거래 데이터 구성
            btc_balance = upbit.get_balance("XRP")
            krw_balance = upbit.get_balance("KRW")
...
def execute_sell(percentage, market_data):
    ...
    try:
        ...
        if btc_balance is None or btc_balance == 0:
            logger.warning("매도 가능한 XRP 잔액 없음")
            return None
        ...
        logger.info(f"매도 주문 시도: {sell_amount:.8f} XRP")
        ...
        if order:
            ...
            # 거래 데이터 구성
            new_btc_balance = upbit.get_balance("XRP")
            krw_balance = upbit.get_balance("KRW")
...
def main():
    ...
    try:
        ...
        # 1분마다 트레이딩 봇 실행
        schedule.every(6).minutes.do(run_scheduled_trading)  #1분간격 ~ 5분간격  > 6분간격
