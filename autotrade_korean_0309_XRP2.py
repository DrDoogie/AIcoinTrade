...
def collect_market_data():
    ...
    try:
        # 1분봉과 5분봉 데이터 수집
        df_1m = pyupbit.get_ohlcv("KRW-XRP", interval="minute1", count=100)
        df_5m = pyupbit.get_ohlcv("KRW-XRP", interval="minute5", count=20)
...
def main():
    ...
    try:
        ...
        # 1분마다 트레이딩 봇 실행
        schedule.every(1).minutes.do(run_scheduled_trading)  #1분간격 ~ 5분간격
