...
def get_current_price(market="NYSE", code="IONQ"):
    ...
    # 응답 데이터 구조 확인
    print("API 응답:", json.dumps(response_json, indent=4, ensure_ascii=False))

    # 'output' 키 확인
    if 'output' in response_json:
        output = response_json['output']
        
        # 'last' 또는 대체할 가격 데이터 확인
        if 'last' in output:
            return float(output['last'])
        elif 'stck_prpr' in output:  # 대체 키 확인
            return float(output['stck_prpr'])
        elif 'ovrs_nmix_prpr' in output:  # 해외 주식의 경우
            return float(output['ovrs_nmix_prpr'])
        else:
            print("⚠️ API 응답에 가격 정보 없음:", output)
            return None
    else:
        print("⚠️ API 응답에 'output' 키 없음:", response_json)
        return None
...
def calculate_rsi(data, periods=14):
    ...
    try:
        ...
        print("RSI 계산을 위한 컬럼:", df.columns)  # 컬럼 확인
        
        # 'last' 키 확인 후 대체할 컬럼 찾기
        price_column = None
        if 'last' in df.columns:
            price_column = 'last'
        elif 'stck_prpr' in df.columns:  # 대체 키
            price_column = 'stck_prpr'
        elif 'ovrs_nmix_prpr' in df.columns:  # 해외 주식의 경우
            price_column = 'ovrs_nmix_prpr'
        
        if not price_column:
            print("⚠️ 'last' 또는 대체할 가격 데이터 없음:", df.head())
            return 50  # 기본값 반환
        
        df['price'] = df[price_column].astype(float)
        ...
        rsi = 100 - (100 / (1 + rs))
        return rsi.iloc[-1] if not rsi.empty else 50  # 기본값 50
