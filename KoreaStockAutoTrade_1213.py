    ...
    for symbol_index, symbol in enumerate(DAILY_SUMMARY_DATA.keys()):
        ...
        if messages:
            ...
            for i, message in enumerate(messages):
                ...
                try:
                    if i == 0:
                        # 심볼 간 구분을 위해 심볼 위에 빈 줄 추가 (모든 심볼에 적용)
...
def main():
    ...
    while True:  # 메인 무한 루프
        try:
            ...
            send_message(f"=== RSI 기반 국내 주식 자동매매 프로그램 시작 ===", level=MESSAGE_LEVEL_IMPORTANT)
            send_message(f"매수 대상 종목: {SYMBOLS}", level=MESSAGE_LEVEL_IMPORTANT)
            send_message(f"기본 매수 비율: 보유 금액의 {BUY_RATIO*100}%", level=MESSAGE_LEVEL_IMPORTANT)
            send_message(f"기본 매수 조건: RSI {RSI_OVERSOLD} 이하 + 현재가가 {MA_PERIOD}일 이동평균 이하", level=MESSAGE_LEVEL_IMPORTANT)
            send_message(f"기본 매도 조건: RSI {RSI_OVERBOUGHT} 이상", level=MESSAGE_LEVEL_IMPORTANT)
            
            # 종목별 설정 표시 (레벨 3이지만 프로그램 시작 시에는 중요하므로 IMPORTANT로 전송)
            send_message("📊 종목별 개별 설정:", level=MESSAGE_LEVEL_IMPORTANT)
            for symbol in SYMBOLS:
                symbol_config = get_symbol_config(symbol)
                send_message(f"  {symbol}: RSI매수 {symbol_config['rsi_oversold']} | RSI매도 {symbol_config['rsi_overbought']} | 익절 {symbol_config['profit_take_percent']}% | 매수비율 {symbol_config['buy_ratio']*100}%", level=MESSAGE_LEVEL_IMPORTANT)
