...
def main():
    ...
    while True:  # 메인 무한 루프
        try:
            ...
            send_message(f"기본 매수 비율: 보유 금액의 {BUY_RATIO*100}%")
            send_message(f"기본 매수 조건: RSI {RSI_OVERSOLD} 이하 + 현재가가 {MA_PERIOD}일 이동평균 이하")
            send_message(f"기본 매도 조건: RSI {RSI_OVERBOUGHT} 이상")
            
            # 종목별 설정 표시
            send_message("📊 종목별 개별 설정:", level=MESSAGE_LEVEL_INFO)
            for symbol in SYMBOLS:
                config = get_symbol_config(symbol)
                send_message(f"  {symbol}: RSI매수 {config['rsi_oversold']} | RSI매도 {config['rsi_overbought']} | 익절 {config['profit_take_percent']}% | 매수비율 {config['buy_ratio']*100}%", level=MESSAGE_LEVEL_INFO)
