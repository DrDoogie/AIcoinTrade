...
try:
    if platform.system() == 'Windows':
        ...
        # Linux/Mac 환경에서는 사용 가능한 폰트 시도
        # 한글 폰트가 없어도 작동하도록 예외 처리
        try:
            available_fonts = [f.name for f in font_manager.fontManager.ttflist]
            # Linux에서 사용 가능한 한글 폰트 우선순위
            korean_fonts = ['NanumGothic', 'NanumBarunGothic', 'Noto Sans CJK KR', 'DejaVu Sans']
            font_set = False
            for font_name in korean_fonts:
                if font_name in available_fonts:
                    ...
                    break
            if not font_set:
                # 한글 폰트가 없으면 기본 폰트 사용 (한글은 깨질 수 있음)
                plt.rcParams['font.family'] = 'DejaVu Sans'
        except Exception:
            # 폰트 목록 조회 실패 시 기본 폰트 사용
            plt.rcParams['font.family'] = 'DejaVu Sans'
