...
SYMBOLS = ["005930","000660","069500", "449450" ]  # 삼성전자, SK하이닉스, ,KODEX 200, PLUS K방산

# RSI 설정
RSI_PERIOD = 14                    # RSI 계산 기간
RSI_OVERSOLD = 30                  # 매수 조건: RSI 이하일 때 (과매도)
RSI_OVERBOUGHT = 70                # 매도 조건: RSI 이상일 때 (과매수)
...
MAX_BUY_COUNT = 3                  # 최대 매수 종목 수

# 체크 주기 설정
RSI_CHECK_INTERVAL = 30            # RSI 체크 간격 (분)
...
def get_stock_balance():
    ...
    try:
        ...
        send_message("====주식 보유잔고====", level=MESSAGE_LEVEL_INFO)
        ...
        time.sleep(0.1)
        send_message("=================", level=MESSAGE_LEVEL_INFO)
...
def main():
    ...
    while True:  # 메인 무한 루프
        try:
            ...
            target_buy_count = min(MAX_BUY_COUNT, len(SYMBOLS))  # 최대 매수 종목 수
            ...
            last_check_time = datetime.datetime.now(tz) - datetime.timedelta(minutes=RSI_CHECK_INTERVAL + 1)
            ...
            while True:
                ...
                # RSI 체크 조건 (설정된 간격마다 또는 첫 실행 시)
                minutes_elapsed = (current_time - last_check_time).total_seconds() / 60
                time_to_check = current_time.minute % RSI_CHECK_INTERVAL == 0 and current_time.second < 30
                
                if force_first_check or (minutes_elapsed >= (RSI_CHECK_INTERVAL - 1) and time_to_check):
                    # 현금 잔고 확인
                    ...
                    # 다음 체크 시간 계산
                    next_check_minutes = RSI_CHECK_INTERVAL - (current_time.minute % RSI_CHECK_INTERVAL)
                    if next_check_minutes == 0:
                        next_check_minutes = RSI_CHECK_INTERVAL
