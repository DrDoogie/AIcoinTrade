...
def wait_for_market_open():
    ...
    try:
        ...
        if not refresh_token():  # 시장 개장 시 토큰 갱신
            send_message("토큰 갱신에 실패했습니다. 2분 후 다시 시도합니다.")
            time.sleep(120)
            refresh_token()
    except Exception as e:
    ...
    while True:  # 메인 무한 루프
        try:
            ...
            if not ACCESS_TOKEN:
                ACCESS_TOKEN = get_access_token()
                if not ACCESS_TOKEN:
                    send_message("토큰 발급 실패, 2분 후 재시도...")
                    time.sleep(120)
                    token_retry_count += 1
            ...
            # 토큰 관련 오류인 경우
            if 'access_token' in error_msg:
                send_message("토큰 오류로 인한 재시작, 2분 후 토큰 재발급을 시도합니다.")
