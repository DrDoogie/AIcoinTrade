...
last_log_time = {}
LOG_INTERVALS = {
    'market_data': 300,      # 시장 데이터 로그는 5분마다
    'hold_status': 600,      # HOLD 상태 로그는 10분마다
    'schedule_info': 1800,   # 스케줄 정보는 30분마다
}
...
TRADING_CONFIG = {
    ...
    'MIN_PROFIT_FOR_SELL': 5.0,       # 최소 수익률 (% 이상일 때만 매도)
...
def log_trade(conn, trade_data):
    ...
    try:
        ...
        conn.commit()
        logger.info("거래 기록 완료: %s at %s원", trade_data['decision'], f"{trade_data['current_price']:,}")
    except Exception as e:
        conn.rollback()
    ...
    try:
        ...
        if (cached_market_data and cache_timestamp and 
            ...
            if should_log('market_data'):
                logger.info(f"캐시된 시장 데이터 사용: RSI {cached_market_data['rsi']:.2f}")
...
def main():
    ...
    try:
        ...
        # 15분마다 트레이딩 봇 실행 (기존 10분에서 15분으로 변경하여 AWS 사용량 저감)
        schedule.every(15).minutes.do(run_scheduled_trading)
        if should_log('schedule_info'):
            logger.info("트레이딩 스케줄 설정 완료 (15분 간격)")
