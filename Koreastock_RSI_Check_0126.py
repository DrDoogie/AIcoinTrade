# RSI 추이 확인 스크립트
# 각 종목별 RSI 값을 조회하고 최근 3일간의 RSI 추이를 리스트로 확인하는 스크립트
...
def print_rsi_list(all_rsi_data):
    """모든 종목의 시간대별 RSI를 리스트로 출력"""
    if not all_rsi_data:
        print("❌ RSI 데이터가 없습니다")
        return
    
    print("\n" + "=" * 100)
    print("📊 종목별 시간대별 RSI 추이 (최근 3일)")
    print("=" * 100)
    
    for symbol, symbol_name, rsi_df in all_rsi_data:
        if rsi_df is None or len(rsi_df) == 0:
            print(f"\n⚠️ {symbol_name}({symbol}): RSI 데이터 없음")
            continue
        
        print(f"\n{'=' * 100}")
        print(f"📈 {symbol_name}({symbol}) - 시간대별 RSI & 현재가")
        print(f"{'=' * 100}")
        print(f"{'날짜':<12} {'시간':<8} {'RSI':<8} {'현재가':<15} {'상태':<15}")
        print("-" * 100)
        
        # 날짜별로 그룹화하여 출력
        rsi_df['date'] = rsi_df['time'].dt.date
        rsi_df['time_str'] = rsi_df['time'].dt.strftime('%H:%M')
        rsi_df['date_str'] = rsi_df['time'].dt.strftime('%m/%d')
        
        # 날짜별로 정렬 (오래된 것부터)
        sorted_df = rsi_df.sort_values('time')
        ...
        for _, row in sorted_df.iterrows():
            ...
            print(f"{date_str:<12} {time_str:<8} {rsi_val:>7.2f}  {price_val:>13,.0f}원  {status:<15}")
        
        # 요약 정보 출력
        print("-" * 100)
        print(f"📊 요약:")
        print(f"   - RSI 범위: {rsi_df['rsi'].min():.2f} ~ {rsi_df['rsi'].max():.2f}")
        print(f"   - 가격 범위: {rsi_df['price'].min():,.0f}원 ~ {rsi_df['price'].max():,.0f}원")
        print(f"   - 최신 RSI: {rsi_df['rsi'].iloc[-1]:.2f} (시간: {rsi_df['time'].iloc[-1].strftime('%m/%d %H:%M')})")
        print(f"   - 최신 가격: {rsi_df['price'].iloc[-1]:,.0f}원")
        
        # 과매수/과매도 구간 분석
        overbought_count = len(rsi_df[rsi_df['rsi'] >= 70])
        oversold_count = len(rsi_df[rsi_df['rsi'] <= 30])
        if overbought_count > 0:
            print(f"   ⚠️ 과매수 구간(RSI≥70): {overbought_count}회")
        if oversold_count > 0:
            print(f"   ✅ 과매도 구간(RSI≤30): {oversold_count}회")
