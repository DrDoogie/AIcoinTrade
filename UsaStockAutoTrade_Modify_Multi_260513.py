...
# KEYB 오류 수정
# 260513 익절 매도 중복 실행 버그 수정 (추세 확인용 1주가 매도되는 문제):
#   1) check_partial_profit_take(): 보유 수량 1주 이하일 때 부분 익절 실행 차단 (추세 확인용 1주 유지)
#   2) check_partial_profit_take(): sell_qty 계산 시 항상 최소 1주 잔여하도록 상한 보정 추가
#   3) 메인 루프: 부분 익절이 실행된 사이클에서는 익절 매도 신호를 건너뜀 (동일 사이클 중복 매도 방지)
...
def check_partial_profit_take(symbol, profit_rate, current_price, quantity, profit_take_percent=None):
    ...
    try:
        ...
        if profit_rate >= partial_profit_target:
            ...
            # [260513 수정] 보유 수량 1주 이하이면 부분 익절 미실행 (추세 확인용 1주 유지)
            if quantity_int <= 1:
                return False

            sell_qty = max(1, int(quantity_int * 0.5))
            # [260513 수정] 매도 후 최소 1주가 잔여하도록 상한 보정 (추세 확인용 1주 유지)
            sell_qty = min(sell_qty, quantity_int - 1)
            if sell_qty <= 0:
                return False
                            ...
                            if buy_signal:
                                ...
                                try:
                                    ...
                                    if symbol in stock_dict:
                                        ...
                                        # [260513 수정] 반환값을 저장하여 동일 사이클 중복 매도 방지에 활용
                                        partial_profit_executed = False
                                        if current_price and current_price > 0 and not is_dividend_no_trade_month(symbol):
                                            ...
                                            partial_profit_executed = check_partial_profit_take(symbol, profit_rate, current_price, int(qty))
                                        ...
                                        if sell_signal:
                                            # [260513 수정] 부분 익절이 이번 사이클에 실행된 경우 익절 매도 건너뜀
                                            # (캐시된 잔고 기준 중복 매도 방지: 부분 익절 후 잔고가 갱신되기 전에 익절 매도가 실행되면 과다 매도 발생)
                                            if partial_profit_executed:
                                                send_message(f"⏭ {symbol} 이번 사이클 부분 익절 실행으로 익절 매도 건너뜀 (다음 사이클에 재확인)", symbol, level=MESSAGE_LEVEL_INFO)
                                            # 배당주 3·6·9·12월: 매도 보류 (배당 수령 위해)
                                            elif is_dividend_no_trade_month(symbol):
